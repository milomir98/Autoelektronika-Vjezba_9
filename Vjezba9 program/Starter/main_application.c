...
void main_demo(void) {
	...
	CPUUsage_TimerHandle = xTimerCreate("Timer", pdMS_TO_TICKS(100), pdTRUE, NULL, checkIdleCountTimerFun);
...
void SerialSend_Task(void* pvParameters) {
	...
	while (1) {
		...
		// PAUZA DA SE DOBIJE VECA ZAUZETOST PROCESORA PRILIKOM SLANJA PORUKE
...
void SerialReceive_Task(void* pvParameters) {
	...
	while (1) {
		...
		// PAUZA DA SE DOBIJE VECA ZAUZETOST PROCESORA PRILIKOM PRIJEMA PORUKE
...
static void checkIdleCountTimerFun(TimerHandle_t tH) {
	...
	if (avg_counter++ == 9) {
		...
		// Idle Hook bez dodatnih taskova je: ~2500000 - Kalibrisati spram raèunara
